...
def plot_combined_streamlines_single_no_streamline(file_path):
    ...
    vec_x_header = 'Uex'    # x方向成分
    vec_y_header = 'Uer'    # y方向成分
    vec_z_header = 'Uep'    # ★ z方向成分（任意の列名を指定可能）

    num_colors = 10
    num_cbar_label = 5
    cbar_label = r'$Electron\ Velocity\ (m/s)$'
    output_name = 'Ue_mag'  # 出力ファイル名（列は使わないが名前はそのまま）
