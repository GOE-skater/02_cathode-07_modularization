...
def plot_combined_streamlines_single_no_streamline(file_path):
    ...
    vec_x_header = 'rhoUix'    # x方向成分
    vec_y_header = 'rhoUir'    # y方向成分
    vec_z_header = 'rhoUip'    # ★ z方向成分（任意の列名を指定可能）

    num_colors = 10
    num_cbar_label = 5
    cbar_label = r'$Ion\ Flux\ (1/m^2s)$'
    output_name = 'rhoUi_mag'  # 出力ファイル名（列は使わないが名前はそのまま）
