...
def plot_combined_streamlines_single_no_streamline(file_path):
    ...
    vector_size = 0.001   # 単位ベクトルに対して掛けるスケール
    # ランダム間引き設定
...
def plot_data(
    ...
    # --- ベクトル成分を取得 ---
    u_vals = data.iloc[:, vec_x_col].to_numpy()  # Uix
    v_vals = data.iloc[:, vec_y_col].to_numpy()  # Uir
    ...
    # --- ベクトルをグリッドへ補間し、ノルムを計算（Z = sqrt(U^2 + V^2)) ---
